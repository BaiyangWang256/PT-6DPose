...
from model.Net_modules import InstanceAdaptiveKeypointDetector, GeometricAwareFeatureAggregator,Reconstructor #,

from PointTransformerV3.model import PointTransformerV3
from model.nocs import NOCS_Predictor, PoseSizeEstimator
...
class Net(nn.Module):
    def __init__(self, cfg):
        ...
        self.pts_extractor =  PointTransformerV3(
                                                    in_channels=3,
                                                    ...
                                                    stride=(1, 1),
                                                    cls_mode=True,
                                                )#PointNet2MSG(radii_list=[[0.01, 0.02], [0.02,0.04], [0.04,0.08], [0.08,0.16]])
    ...
    def forward(self, inputs):
        ...
        #print(pts.shape)
        batch1 = torch.arange(b).unsqueeze(1).repeat(1, 1024).reshape(-1).cuda()
        #print(batch1.shape)
        data_dict = {
            "coord": pts.reshape(-1, 3),
            "feat": pts.reshape(-1, 3),
            "batch": batch1,
            "grid_size": 0.01,
        }
        #print(data_dict)
        #print(data_dict["coord"].shape, data_dict["feat"].shape, data_dict["batch"].shape, )
        out = self.pts_extractor(data_dict) # b, c, n
        ...
        C = feat.shape[1]
        dense = torch.zeros(b, 1024, C, device=feat.device)
        start = 0
        for i in range(b):
            end = offset[i]
            cnt = end - start

            dense[i, :cnt, :] = feat[start:end, :]
            start = end


        pts_local = dense.permute(0, 2, 1).contiguous()

        batch_kpt_query, heat_map = self.IAKD(rgb_local, pts_local)

        kpt_3d = torch.bmm(heat_map, pts)#print(kpt_3d.shape)
        kpt_feature = torch.bmm(heat_map, torch.cat((pts_local, rgb_local), dim=1).transpose(1, 2))#print(kpt_feature.shape)#torch.Size([20, 96, 256])


        kpt_feature = self.GAFA(kpt_feature, kpt_3d.detach(), torch.cat((pts_local, rgb_local), dim=1).transpose(1, 2), pts)#print(kpt_feature.shape)#torch.Size([20, 96, 256])
