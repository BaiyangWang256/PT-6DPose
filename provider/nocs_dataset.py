...
class TrainingDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        """ret_dict 包含训练 RGB-D 点云网络所需的完整信息。
        
        pts 是从物体表面 mask 区域采样的点云，形状为 [N, 3]，在相机坐标系下；
        rgb 是对应区域的 RGB 图像，形状 [3, image_size, image_size]，经过增强和归一化；
        choose 是采样点在缩放后图像中的平面索引，形状 [N]，用于建立点云与图像的对应关系；
        category_label 是物体类别标签，形状 [1]，0 索引；
        model 是物体的标准化 3D 模型点云，形状 [M, 3]，作为训练参考；
        qo 是经过平移、归一化和旋转后的点云，形状 [N, 3]，用于与模型对齐计算损失；
        translation_label 和 rotation_label 分别是物体在相机坐标系下的平移向量 [3] 和旋转矩阵 [3, 3]；
        size_label 是物体尺度 [3]，用于归一化点云；
        pc_mask 是布尔掩码 [N]，标记每个点是否靠近模型表面，用于选择有效点计算损失。整个字典将点云、RGB、类别、模型及位姿信息整合，构成网络训练所需的完整输入和标签。"""
