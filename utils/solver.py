...
class Solver(gorilla.solver.BaseSolver):
    ...
    def solve(self, resume_path=None):
        # --------------------------
        ...
        while self.epoch <= self.cfg.max_epoch:
            ...
            # Save checkpoint (model + optimizer + scheduler + epoch + iter)
    ...
    def save_checkpoint(self):
        ...
        ckpt_path = os.path.join(self.cfg.ckpt_dir, f"epoch_{self.epoch}.pt")
    ...
    def load_checkpoint(self, path):
        ...
        if ckpt["scheduler"] is not None and self.lr_scheduler is not None:
            self.lr_scheduler.load_state_dict(ckpt["scheduler"])

        self.epoch = ckpt["epoch"] + 1
        self.iter = ckpt["iter"]

        # IMPORTANT — resume LR scheduler position
        if self.lr_scheduler is not None:
            self.lr_scheduler.last_epoch = self.iter
